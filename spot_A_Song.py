...
class Song:
    ...
    def search_song_lyrics(self, lyrics_search, artist_search):
        ...
        lyrics_match = [match.group('name') for match in regex_matches if 
                        lyrics_search in match.group('lyrics')]
        return lyrics_match
        #Use list comprehension to pull matching artists        
        #self.artist_match = [match.group('name') for match in regex_matches 
...
def main():
    # Get input from user for song lyrics and artist name
     ...
     print(f"The possible matching song(s) are: {x.search_song_lyrics(lyrics_search, artist_search)[0]}")          
